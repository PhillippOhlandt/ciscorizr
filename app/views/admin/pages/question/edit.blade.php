...
@section('content')
	...
	<div class="row">
		<div class="col-md-12"><h2><span class="color color_font color_font-1">Edit Question:</span> {{ $question->title }}</h2></div>
	</div>

	@include('admin.components.errormessages', ['errors' => $errors])

	{{ Form::open(array('route' => array('question-edit', $course->id, $chapter->id, $question->id))) }}
		<div class="row">
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
			    <div class="form-group">
			    	{{ Form::label('granted_time', 'Granted Time (s)') }}
				    {{ Form::text('granted_time', $question->granted_time, array('class' => 'form-control', 'placeholder'=>'Granted time')) }}
			    </div>
		    </div>
		</div>
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
		    	<div class="form-group">
			    	<div class="checkbox">
					   @if($can_make_visible)
							    <label>{{ Form::checkbox('is_visible', 'is_visible', $question->is_visible) }}Visible</label>
						@else
							You need at least <span class="color color_font color_font-1"><b>4</b></span> answers to make this question visible
						@endif
					</div>
			    </div>
		    </div>
