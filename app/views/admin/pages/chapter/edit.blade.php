...
@section('content')
	...
	<div class="row">
		<div class="col-md-12"><h2><span class="color color_font color_font-1">Edit Chapter:</span> {{ $chapter->title }}</h2></div>
	</div>

	@include('admin.components.errormessages', ['errors' => $errors])

	{{ Form::open(array('route' => array('chapter-edit', $course->id, $chapter->id))) }}
		<div class="row">
		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
		    	<div class="form-group">
			    	<div class="checkbox">
					   @if($can_make_visible)
							    <label>{{ Form::checkbox('is_visible', 'is_visible', $chapter->is_visible) }}Visible</label>
						@else
							You need at least <span class="color color_font color_font-1"><b>10</b></span> visible questions to make this chapter visible
						@endif
					</div>
			    </div>
		    </div>
